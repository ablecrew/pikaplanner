...
export default function MealPlansPage() {
  ...
  const fetchCurrentPlan = async (uid?: string | null) => {
    ...
    try {
      ...
      const latestPlan = (data as MealPlan) || null
      setPlan(latestPlan)
      return latestPlan
    } catch (err: unknown) {
      console.error('Meal plan fetch failed:', err)
  ...
  const fetchUserPreferences = async (uid: string) => {
    try {
      const { data, error: prefError } = await supabase
        ...
        .maybeSingle()
      if (!prefError && data) {
        setUserPreferences(data as any)
      }
    } catch (err) {
  ...
  const generateAIPlan = async () => {
    ...
    try {
      const { error: genError } = await supabase.functions.invoke('generate-meal-plan', {
        body: {
          userId,
          cuisinePreferences: userPreferences?.cuisine_preferences ?? ['kenyan', 'healthy'],
          mealTypes: ['breakfast', 'lunch', 'dinner'],
          allergies: userPreferences?.dietary_preferences?.filter((d: string) => d === 'Gluten-Free' || d === 'Dairy-Free') ?? [],
          budgetPerDay: userPreferences?.budget_range ? parseInt(userPreferences.budget_range.split('-')[1] || '1000') : 1000,
          targetCalories: 2000,
        },
      })
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        await Promise.all([fetchCurrentPlan(uid), fetchUserPreferences(uid)])
      } catch (err) {
  ...
  const dayMap = useMemo(() => {
    const map: Record<string, MealEntry | null> = {
      Mon: null,
      Tue: null,
      Wed: null,
      Thu: null,
  ...
  const filledDays = useMemo(() => {
    return DAYS.filter((day) => dayMap[day] !== null).length
  }, [dayMap])
  const totalCalories = useMemo(() => {
    return DAYS.reduce((sum, day) => {
      const entry = dayMap[day]
      const cal = entry?.meals?.calories_per_serving ?? entry?.calories ?? 0
      return sum + cal
    }, 0)
  }, [dayMap])
  const totalPrepTime = useMemo(() => {
    return DAYS.reduce((sum, day) => {
      const entry = dayMap[day]
      const prep = entry?.meals?.prep_time_minutes ?? 0
      const cook = entry?.meals?.cook_time_minutes ?? 0
            ...
            <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-sm font-bold text-white/90 backdrop-blur">
                  <Calendar className="h-4 w-4 text-[#32CD32]" />
                  AI Meal Planning
                </div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <button
                  ...
                  disabled={generating}
                  className="inline-flex items-center gap-2 rounded-xl bg-[#f97316] px-6 py-3 text-sm font-black uppercase tracking-wide text-white transition hover:bg-[#ea580c] disabled:opacity-60"
                >
        ...
        <section className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-black text-slate-900">{totalCalories > 0 ? totalCalories : '-'}</p>
                ...
                return (
                  ...
                  >
                    <div className="mb-3 flex items-center justify-between">
                      <span className={`text-[10px] font-black uppercase tracking-widest ${isFilled ? 'text-[#126e3d]' : 'text-gray-400'}`}>
                        {day}
                      </span>
                      {isFilled && (
                        <MealIcon className="h-4 w-4 text-[#f97316]" />
                      )}
                    </div>
                    {isFilled ? (
                      <div className="flex-1 space-y-2">
                        ...
                        <div className="mt-2 space-y-1.5 text-[11px] font-semibold text-gray-500">
                          ...
                          {totalMinutes > 0 && (
                            <p className="flex items-center gap-1.5">
                              <Clock size={12} className="text-[#126e3d]" />
                              {totalMinutes} min
                            </p>
                          )}
                          {cuisine && (
                            <p className="flex items-center gap-1.5">
                              <Zap size={12} className="text-[#32CD32]" />
                              {cuisine}
                            </p>
                          )}
                        </div>
        ...
        {!loading && filledDays === 0 && (
          <section className="mb-8 rounded-3xl border border-gray-100 bg-white p-10 text-center shadow-sm">
            ...
            <button
              onClick={() => void generateAIPlan()}
              disabled={generating}
              className="mt-6 inline-flex items-center gap-2 rounded-xl bg-[#126e3d] px-6 py-3 text-sm font-black text-white transition hover:bg-[#0f5c33] disabled:opacity-60"
            >
              {generating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
        ...
        {filledDays > 0 && (
          <section className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <button
              onClick={() => void generateAIPlan()}
              disabled={generating}
              className="flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition hover:border-emerald-200 hover:shadow-md disabled:opacity-60"
            >
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#f0fdf4] text-[#126e3d]">
                {generating ? <Loader2 className="h-5 w-5 animate-spin" /> : <Sparkles size={22} />}
              </div>
              <div>
                <p className="text-sm font-black text-slate-900">Regenerate Plan</p>
                <p className="text-xs font-medium text-gray-500">Get fresh meal ideas instantly</p>
              </div>
