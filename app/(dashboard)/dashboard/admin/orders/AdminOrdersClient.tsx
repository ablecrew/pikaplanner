...
import {
  ...
  AlertCircle, CheckCircle2, RefreshCw, Package, Clock,
  Truck, MapPin, Phone, Mail, Calendar, CreditCard, Hash,
...
import { 
  Order, OrderStatus, fetchAdminOrdersData, 
...
const StatCard = memo(function StatCard({ label, value, icon: Icon, color, sub }: { label: string; value: string | number; icon: React.ComponentType<{ size?: number; className?: string }>; color: string; sub?: string }) {
  return (
...
const OrderRow = memo(function OrderRow({ 
  order, onView, onConfirm, actionLoadingId 
}: { 
  order: Order
  onView: (order: Order) => void
  onConfirm: (type: string, order: Order) => void
  ...
  return (
    <tr className="border-t border-gray-50 hover:bg-gray-50/50 transition-colors">
      <td className="px-5 py-4">
        ...
        {order.mpesaRef && <p className="text-[10px] text-gray-400 mt-0.5 truncate max-w-[140px]">{order.mpesaRef}</p>}
      </td>
      <td className="px-5 py-4">
      ...
      <td className="px-5 py-4">
        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold ${order.paymentStatus === 'paid' ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-600'}`}>
          {order.paymentMethod}
        </span>
      </td>
      ...
      <td className="px-5 py-4">
        <ActionMenu actions={[
          { label: 'View Details', icon: <Eye size={14} />, onClick: () => onView(order) },
          ...(order.status === 'Pending' || order.status === 'Processing'
            ? [{ label: 'Mark Complete', icon: <CheckCircle size={14} />, onClick: () => onConfirm('complete', order) }]
            : []),
          ...(order.status !== 'Cancelled' && order.status !== 'Completed'
            ? [{ label: 'Cancel Order', icon: <XCircle size={14} />, onClick: () => onConfirm('cancel', order), variant: 'warning' as const }]
            : []),
          { label: 'Delete', icon: <Trash2 size={14} />, onClick: () => onConfirm('delete', order), variant: 'danger' as const },
        ]} />
      </td>
    </tr>
  )
...
export default function AdminOrdersClient({ initialOrders }: { initialOrders: Order[] }) {
  ...
  const refreshOrders = useCallback(async () => {
    setLoading(true)
    try {
      const data = await fetchAdminOrdersData()
      setOrders(data)
    } catch (err) {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      revenue: `KES ${revenue.toLocaleString('en-KE', { minimumFractionDigits: 2 })}`,
  ...
  const filtered = useMemo(() => {
    ...
    return orders.filter(o => {
      const matchSearch = !q || o.orderNumber.toLowerCase().includes(q) || o.customer.toLowerCase().includes(q) || o.vendor.toLowerCase().includes(q) || o.mpesaRef.toLowerCase().includes(q) || o.customerPhone.includes(q)
  ...
  const handleStatusChange = async (order: Order, newStatus: OrderStatus) => {
    ...
    try {
      ...
      await refreshOrders() // Revert on error
    } finally {
  ...
  const handleExport = () => {
    const rows = [['Order #', 'Customer', 'Vendor', 'Amount', 'Payment', 'Status', 'Date'], ...filtered.map(o => [o.orderNumber, o.customer, o.vendor, o.totalAmount, o.paymentMethod, o.status, o.date])]
    const csv = rows.map(r => r.map(v => `"${String(v).replace(/"/g, '""')}"`).join(',')).join('\n')
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url; a.download = `pikaplan-orders-${new Date().toISOString().split('T')[0]}.csv`; a.click()
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="font-poppins">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-6">
        ...
        <button onClick={refreshOrders} disabled={loading} className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition disabled:opacity-50">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 border-b border-gray-100">
          <div className="flex items-center gap-3 flex-1 max-w-md w-full">
            <div className="relative flex-1">
              ...
              <input value={search} onChange={(e) => startTransition(() => setSearch(e.target.value))} placeholder="Search by order #, customer, vendor..." className="w-full pl-9 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-100 focus:border-emerald-400 transition text-gray-900" />
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            {['All', 'Pending', 'Processing', 'Completed', 'Cancelled'].map((s) => (
              <button key={s} onClick={() => startTransition(() => setStatusFilter(s))} className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all ${statusFilter === s ? 'bg-emerald-500 text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{s}</button>
            ))}
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} className="px-5 py-16 text-center"><div className="flex flex-col items-center gap-3"><Loader2 size={28} className="animate-spin text-emerald-500" /><p className="text-sm text-gray-400 font-medium">Loading orders...</p></div></td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={8} className="px-5 py-16 text-center"><div className="flex flex-col items-center gap-3"><ShoppingBag size={36} className="text-gray-300" /><p className="text-sm text-gray-400 font-medium">No orders found</p></div></td></tr>
              ) : (
                filtered.map((order) => (
                  <OrderRow 
                    key={order.id} 
                    order={order} 
                    onView={(o) => { setSelected(o); setModalMode('view') }} 
                    onConfirm={(type, o) => setConfirm({ type, order: o })} 
        ...
        {selected && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <span className={`px-3 py-1.5 rounded-full text-xs font-bold border ${statusStyle(selected.status)}`}>{selected.status}</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 bg-gray-50 rounded-xl p-5">
              {[
                ...
                <div key={label} className="flex items-start gap-2.5">
                  ...
                  <div><p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wide">{label}</p><p className="text-sm font-semibold text-gray-800">{value}</p></div>
                </div>
              ))}
            </div>
          </div>
      ...
      <ConfirmDialog
        isOpen={!!confirm}
        title={confirm?.type === 'delete' ? 'Delete Order' : confirm?.type === 'complete' ? 'Complete Order' : 'Cancel Order'}
        message={confirm?.type === 'delete' ? `Permanently delete order ${confirm?.order.orderNumber}? This cannot be undone.` : confirm?.type === 'complete' ? `Mark order ${confirm?.order.orderNumber} as completed?` : `Cancel order ${confirm?.order.orderNumber}? This will notify the customer.`}
        confirmLabel={confirm?.type === 'delete' ? 'Delete' : confirm?.type === 'complete' ? 'Mark Complete' : 'Cancel Order'}
        confirmVariant={confirm?.type === 'delete' ? 'danger' : confirm?.type === 'complete' ? 'primary' : 'warning'}
